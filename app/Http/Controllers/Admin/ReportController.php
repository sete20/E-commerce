...
class ReportController extends Controller
{
    ...
    public function SearchByYear(Request $request)
    {
        ...
        $total = DB::table('orders')->where('status', 3)->where('year', $request->year)->sum('total');

        $order = DB::table('orders')->where('status', 3)->where('year', $request->year)->get();
    ...
    {
        $total = DB::table('orders')->where('status', 3)->where('month', $request->month)->sum('total');

        $order = DB::table('orders')->where('status', 3)->where('month', $request->month)->get();
